...
type Components struct {
	...
	BypassIPsInput            *widget.Entry
	BypassLoadButton          *widget.Button
	BypassCheckButton         *widget.Button
	BypassApplyButton         *widget.Button
	DomainsInput              *widget.Entry
	DomainsLoadButton         *widget.Button
	DomainsCheckButton        *widget.Button
	DomainsApplyButton        *widget.Button
	...
	bypassTabContent          fyne.CanvasObject
	domainsTabContent         fyne.CanvasObject
...
func NewComponents() *Components {
	...
	c := &Components{
		...
		BypassIPsInput:            widget.NewMultiLineEntry(),
		BypassLoadButton:          widget.NewButton("Load from router", nil),
		BypassCheckButton:         widget.NewButton("Check list", nil),
		BypassApplyButton:         widget.NewButton("Apply to router", nil),
		DomainsInput:              widget.NewMultiLineEntry(),
		DomainsLoadButton:         widget.NewButton("Load from router", nil),
		DomainsCheckButton:        widget.NewButton("Check list", nil),
		DomainsApplyButton:        widget.NewButton("Apply to router", nil),
	...
	c.RouterImage.Hide()
	c.RamInfoLabel.Alignment = fyne.TextAlignTrailing
	c.RamInfoLabel.Hide()
	...
	c.BypassIPsInput.SetPlaceHolder("One IP or subnet per line, # for comments:\n1.2.3.4\n10.10.0.0/16\n2a02:6b8::/32")
	c.BypassApplyButton.Disable()
	// Пустой список тоже применим (очищает обход), поэтому Check не блокируем,
	// но после любой правки требуем повторную проверку
	c.BypassIPsInput.OnChanged = func(string) {
		c.BypassApplyButton.Disable()
	}

	c.DomainsInput.SetPlaceHolder("One domain per line, leading dot matches subdomains, # for comments:\nrutracker.org\n.youtube.com")
	c.DomainsApplyButton.Disable()
	c.DomainsInput.OnChanged = func(string) {
		c.DomainsApplyButton.Disable()
	}
	...
	c.bypassTabContent = c.buildBypassTab()
	c.domainsTabContent = c.buildDomainsTab()
...
func (c *Components) buildBypassTab() fyne.CanvasObject {
	inputScroll := container.NewScroll(c.BypassIPsInput)
	inputScroll.SetMinSize(fyne.NewSize(600, 220))

	bypassButtons := container.NewGridWithColumns(3,
		c.BypassLoadButton,
		c.BypassCheckButton,
		c.BypassApplyButton,
	)

	bypassLog := container.New(layout.NewVBoxLayout(),
		c.LogScroll,
	)

	return container.NewVBox(
		widget.NewLabel("Bypass IPs / subnets (one per line, # for comment):"),
		inputScroll,
		bypassButtons,
		bypassLog,
	)
...
func (c *Components) buildDomainsTab() fyne.CanvasObject {
	inputScroll := container.NewScroll(c.DomainsInput)
	inputScroll.SetMinSize(fyne.NewSize(600, 220))

	domainsButtons := container.NewGridWithColumns(3,
		c.DomainsLoadButton,
		c.DomainsCheckButton,
		c.DomainsApplyButton,
	)

	domainsLog := container.New(layout.NewVBoxLayout(),
		c.LogScroll,
	)

	return container.NewVBox(
		widget.NewLabel("VPN domains (one per line, leading dot matches subdomains):"),
		inputScroll,
		domainsButtons,
		domainsLog,
	)
...
func (c *Components) BuildUI() fyne.CanvasObject {
	c.tabbedContainer = container.NewAppTabs(
		...
		container.NewTabItem("Bypass IPs", c.bypassTabContent),
		container.NewTabItem("Domains", c.domainsTabContent),
	)
